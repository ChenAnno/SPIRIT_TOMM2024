    ...
    def __getitem__(self, index):
        try:
            if self.mode == 'relative':
                ...
                reference_feature_path = self.base_path + 'fashion-iq/fashion_local/' + f"{reference_name}.pth"
                reference_feature = torch.load(reference_feature_path)
                ...
                if self.split == 'train':
                    ...
                    target_feature_path = self.base_path + 'fashion-iq/fashion_local/' + f"{target_name}.pth"
                    target_feature = torch.load(target_feature_path)
                    return reference_image, target_image, image_captions, reference_feature, target_feature

                elif self.split == 'val':
                    target_name = self.triplets[index]['target']
                    target_feature_path = self.base_path + 'fashion-iq/fashion_local/' + f"{target_name}.pth"
                    target_feature = torch.load(target_feature_path)
                    return reference_name, target_name, image_captions, reference_feature, target_feature
                ...
                local_feature_path = self.base_path + 'fashion-iq/fashion_local/' + f"{image_name}.pth"
...
if __name__ == '__main__':
    train_dataset = FashionIQDataset(split="train", dress_types=["dress"])
    train_loader = DataLoader(dataset=train_dataset, batch_size=4, collate_fn=collate_fn, drop_last=True,
                              shuffle=True)
